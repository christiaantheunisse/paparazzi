...
int opencv_main(char *img, int width, int height, int do_pause) {
    if (do_pause > 0) {
//        printf("img processing is pausing");
        // Reset detections
        for (int n = 0; n < N_DIRBLOCKS; n++) {
            detection_history[n] = 0;
        }
        // Save empty grayscale for OF
        Mat empty_mat;
        ...
        cv::Rect cropped_region(gray_resized.cols - param_CROP_HEIGHT - param_CROP_Y, param_CROP_X, param_CROP_HEIGHT,
                                param_CROP_WIDTH);
